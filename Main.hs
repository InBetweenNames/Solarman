...
interpret' input = do
    ...
    if firstpass == "BLANKVALNOTUSED" then do
        ...
        outs <- mapM (\i -> TypeAg2.getReducedTriplestore Local.localData (TypeAg2.flattenGetts i) >>= \rtriples -> return $ TypeAg2.getSem i rtriples) interpretations --TODO: this is a code smell -- needs to be abstracted -- looks like SemFunc
